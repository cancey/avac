...
! Also calls movetopo if topography might be moving.
    ...
    real(kind=8) :: h,u,v
    real(kind=8) :: tolfb1,tolfb2
    real(kind=8) :: xfront1, xfront2, xback1, xback2, x, posf(1:mx), dep(1:mx)
    ...
    do i=1,mx
      posf(i) = xlower + (i-0.5d0)*dx
      dep(i) = q(1,i,1)
    enddo
    
    where (dep .le. 0.01) posf = 0.
    
      write(out_unit, "(50e26.16)") t, maxval(posf)
