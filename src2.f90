                ...
                if (q(1,i,j) < depth_tolerance) then
                    ...
                    if (q(1,i,j) <= friction_depth) then
                       ...
                       if (q(3, i, j)<0.) then
                        ...
                        if (sgnx /= 0.) then
                        q(2, i, j) = q(2, i, j)/(dgamma + muv*g*q(1,i,j)*dt*cos(thetax)*sgnx/q(2, i, j))
                        endif
                        if (sgny /= 0.) then
                        q(3, i, j) = q(3, i, j)/(dgamma + muv*g*q(1,i,j)*dt*cos(thetay)*sgny/q(3, i, j))
                        endif
                        if ((thetac<muv) .and. (abs(q(2, i, j))<ucrit*q(1, i, j)) .and. (abs(q(3, i, j))<ucrit*q(1, i, j))) then
                        q(2, i, j)=0.
                        q(3, i, j)=0.
                        endif
                    endif
