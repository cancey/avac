...
subroutine valout(level_begin, level_end, time, num_eqn, num_aux)
    ...
    integer, parameter :: out_raster = 60
    ...
    logical :: out_aux
    character(len=11) :: file_name(6)

    real(kind=8) :: h, hu, hv, eta, xx, yy, vit
            ...
            ! Export to raster format
                    ...
                    do j = num_ghost + 1, num_cells(2) + num_ghost
                        do i = num_ghost + 1, num_cells(1) + num_ghost
                            ...
                            if (level == 1) then
                                write(out_raster, "(50e26.16)") xx, yy, h, hu, hv, vit
                             end if
                        end do
                        write(out_unit, *) ' '
                        if (level == 1) then
